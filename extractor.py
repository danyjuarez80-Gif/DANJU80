...
def obtener_php_links(page, url_pagina):
    """Visita la pagina del canal y extrae TODOS los links PHP de opciones."""
    try:
        ...
        vistos = set()
        for l in links:
            if "/live" in l and ".php" in l and "tvplusgratis2" in l and l not in vistos:
                php_links.append(l)
                vistos.add(l)
        return php_links
...
def capturar_m3u8(page, url_php):
    """Abre el PHP e intercepta el m3u8 de las requests de red."""
    capturados = []
...
def generar_lista():
    ...
    # Scrapear todos los canales con Playwright
    nuevos_canales = []
    with sync_playwright() as p:
        ...
        for canal in CANALES:
            print(f"\n📺 {canal['nombre']}")
            php_links = obtener_php_links(page, canal["pagina"])

            if not php_links:
                print(f"  ❌ Sin opciones")
                continue

            for i, php_url in enumerate(php_links, 1):
                print(f"  Op{i}: {php_url}")
                m3u8 = capturar_m3u8(page, php_url)
                if m3u8 and m3u8 not in canales_vistos:
                    nombre = f"{canal['nombre']} Op{i}"
                    nuevos_canales.append(f'#EXTINF:-1 group-title="Deportes",{nombre}')
